...
*   Tilting in the z-axis when past a threshold will move the drone left or right.
*
...
void orientationCallback(const geometry_msgs::Quaternion::ConstPtr msg)
{
  ...
  if (msg->y < -0.2) {
    desired_yaw = desired_yaw - speed/3;
  } else if (msg->y > 0.2) {
    desired_yaw = desired_yaw + speed/3;
  }
...
{
  ros::init(argc, argv, "oculus_control_alternative");
  ros::NodeHandle nh;
